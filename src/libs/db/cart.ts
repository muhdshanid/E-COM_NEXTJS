...
export const getCart = async (): Promise<ShoppingCart | null> => {
    ...
    if(session){
        ...
        const localCartId = cookies().get("localCartId")?.value
         cart = localCartId ? await prisma.cart.findUnique({where: {id: localCartId},
        include: { items: {
            include: {product: true}
        }}})
        : null
...
export const mergeCart = async (id: string) => {

    const localCartId = cookies().get("localCartId")?.value
    const localCart = localCartId ? await prisma.cart.findUnique({where: {id: localCartId},
        include: { items: true}})
        : null

    if(!localCart) return

    const userCart = await prisma.cart.findFirst({where: {
        userId: id
    }, include: {items: true}})
